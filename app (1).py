...
monthly_data = filtered_day_df.set_index('dteday').resample('M')['cnt'].mean().reset_index()
fig_monthly = px.line(
    monthly_data,
    x='dteday',
    y='cnt',
    title='Monthly Average Rentals'
)
st.plotly_chart(fig_monthly, use_container_width=True)

# Footer
st.markdown('---')
st.markdown('Data source: [Bike Sharing Dataset](https://www.kaggle.com/datasets/lakshmi25npathi/bike-sharing-dataset)')
